...
def ann(anno, vcfh):
    ...
    anot_descr = [
        x.replace(" ", "")
        for x in p_straight.split(header["INFO"]["ANN"]["Description"].split("'")[1])[
            1::2
        ]
    ]
    # split
    ...
    for allele in alleles:
        ...
        if allele_anot["Annotation"] == "intergenic_region":
            ...
            annlist.append(
                (
                    allele_anot["Annotation"],
                    allele_anot["Gene_ID"],
                    allele_anot["Gene_Name"],
                    allele_anot["Distance"],
                    allele_anot["HGVS.c"],
                    allele_anot["HGVS.p"],
                )
            )
...
def samples(sample, name, vcfh, FORMAT):
    ...
    for i in range(len(sample)):
        ...
        if "RD" in alleleh:
            ...
            vcfh[name[i]][vcfh["REF"]]["freq"] = isfloat(alleleh["RD"]) / (
                isfloat(alleleh["RD"]) + isfloat(alleleh["AD"])
            )
            vcfh[name[i]][vcfh["ALT"]]["count"] = isfloat(alleleh["AD"])
            vcfh[name[i]][vcfh["ALT"]]["freq"] = isfloat(alleleh["AD"]) / (
                isfloat(alleleh["RD"]) + isfloat(alleleh["AD"])
            )
        else:
            vcfh[name[i]]["coverage"] = isfloat(alleleh["DP"])
            vcfh[name[i]][vcfh["REF"]]["count"] = isfloat(alleleh["DP"]) - isfloat(
                alleleh["AD"]
            )
            vcfh[name[i]][vcfh["REF"]]["freq"] = vcfh[name[i]][vcfh["REF"]][
                "count"
            ] / isfloat(alleleh["DP"])
            vcfh[name[i]][vcfh["ALT"]]["count"] = isfloat(alleleh["AD"])
            vcfh[name[i]][vcfh["ALT"]]["freq"] = isfloat(alleleh["AD"]) / isfloat(
                alleleh["DP"]
            )
...
for l in load_data(sys.argv[1]):
    ...
    for annot in result["INFO"]["ANN"]:
        print(datah[ID] + "\t" + "\t".join([str(x) for x in annot]))
